...
public class OracleDAO implements FactoryDAO {
    private final DBConnect conexionDB;
    public OracleDAO() {
        this.conexionDB = DBConnect.getInstancia();
    }
    public OracleDAO(DBConnect conexionDB) {
        if (conexionDB != DBConnect.getInstancia()) {
            System.out.println("Advertencia: OracleDAO no está usando la instancia singleton");
        }
